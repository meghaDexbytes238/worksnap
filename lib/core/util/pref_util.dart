...
class PrefUtils {
  ...
  Future<int> readInt(keys) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final key = keys;
      debugPrint('read key: $keys');
      final value = prefs.getInt(key) ?? -1;
      debugPrint('read value: $keys $value');
      return value;
    } catch (e) {
      debugPrint("$e");
      return -1;
...
//is_user_loggedin
  ...
  Future<bool> readBool(keys) async {
    try {
      ...
      debugPrint('read key and value : $keys $value');
      return value;
    } catch (e) {
      debugPrint("$e");
      return false;
  ...
  saveBool(keys, values) async {
    try {
      ...
      final key = keys;
      final value = values;
      debugPrint('value for save : $values');
      prefs.setBool(key, value);
      debugPrint('saved key : $key $value');
    } catch (e) {
      debugPrint("$e");
    }
  }
